document.addEventListener("DOMContentLoaded", async () => {
  ...
  const on = (el, evts, fn, opts) => {
    if (!el) return;
    evts.split(" ").forEach(e => el.addEventListener(e, fn, opts || { passive:false }));
  };
  const toast = msg => {
  ...
  if (overlay && yes && no) {
    ...
    on(yes, "click touchstart", e => { e.preventDefault(); overlay.style.display="none"; });
    on(no,  "click touchstart", e => { e.preventDefault(); alert("Sorry, you must be 21+ to enter."); location.href="https://google.com"; });
  }
  ...
  // ---------- PRODUCTS ----------
  const PLACEHOLDER_IMG = "data:image/svg+xml," + encodeURIComponent("<svg xmlns='http://www.w3.org/2000/svg' width='300' height='200'><rect width='100%' height='100%' fill='#0b0b0b'/><text x='50%' y='50%' fill='#ff8c00' font-size='18' text-anchor='middle'>No Image</text></svg>");
  ...
  async function addCard(p){
    ...
    on(card.querySelector(".add-btn"), "click touchstart", ()=>addToCart({id:p.id,name:p.name,price,img}));
  ...
  const addToCart=(item)=>{
    ...
    saveCart();renderCart();toast("Added");
  };
  ...
  async function startIDScan(){
    try{
      ...
      console.error(e);alert("Camera failed. Check permissions.");
    }
  }
  ...
  on(checkoutBtn,"click touchstart",async e=>{
    if(!idVerified){e.preventDefault();toast("Please scan ID first.");return startIDScan();}
    toast("✅ Checkout ready");
    // add your EmailJS or Square code here...
  });
  ...
  console.log("🚀 LBizzo ready, buttons fixed.");
});
